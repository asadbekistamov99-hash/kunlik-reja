...
class CommandParser(private val clock: () -> LocalDateTime = { LocalDateTime.now() }) {
    ...
    fun parse(text: String): ParsedCommand {
        ...
        // 3. Explicit dates
        if (date == null) {
            ISO_DATE_RE.find(working)?.let { m ->
                date = runCatching { LocalDate.of(m.groupValues[1].toInt(), m.groupValues[2].toInt(), m.groupValues[3].toInt()) }.getOrNull()
                if (date != null) working = working.removeRange(m.range)
            }
        }
        if (date == null) {
            MONTH_DATE_RE.find(working)?.let { m ->
                val day = m.groupValues[1].toInt()
                val month = MONTHS.indexOfFirst { m.groupValues[2].startsWith(it) } + 1
                date = runCatching { rollForward(LocalDate.of(now.year, month, day), now.toLocalDate()) }.getOrNull()
                if (date != null) working = working.removeRange(m.range)
            }
        }
        if (date == null) {
            DOTTED_DATE_RE.find(working)?.let { m ->
                val day = m.groupValues[1].toInt()
        ...
        if (date == null) {
            IN_DAYS_RE.find(working)?.let { m ->
                date = now.toLocalDate().plusDays(m.groupValues[1].toLong())
                working = working.removeRange(m.range)
            }
        }
        if (date == null) {
            for ((re, offset) in RELATIVE_DAYS) {
                val m = re.find(working) ?: continue
        ...
        if (date == null) {
            WEEKDAY_RE.find(working)?.let { m ->
                val dow = WEEKDAYS.getValue(m.groupValues[1])
                val next = m.groupValues[0].contains("keyingi")
                val base = now.toLocalDate()
                date = if (next) base.plusWeeks(1).with(TemporalAdjusters.nextOrSame(dow))
