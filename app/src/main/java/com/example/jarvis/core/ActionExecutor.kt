...
class ActionExecutor(
    ...
    private suspend fun listPending(intent: ResolvedIntent): JarvisResponse {
        ...
        if (overdue > 0) sb.append(", ulardan $overdue tasi muddati o'tgan")
        sb.append(". ")
        pending.take(5).forEachIndexed { i, t -> sb.append("${i + 1}. ${t.title}. ") }
        return JarvisResponse(sb.toString().trim(), intent.type,
            card = ResponseCard("Tugallanmagan ishlar", pending.take(30).map { "${it.dateString} ${it.timeString} — ${it.title}" }))
    }
    ...
    private fun readNotifications(intent: ResolvedIntent): JarvisResponse {
        ...
        return JarvisResponse(com.example.jarvis.integrations.Notifications.describe(items), intent.type,
    ...
    private suspend fun emailRead(intent: ResolvedIntent): JarvisResponse {
        ...
        return JarvisResponse(Gmail.describe(mails), intent.type,
