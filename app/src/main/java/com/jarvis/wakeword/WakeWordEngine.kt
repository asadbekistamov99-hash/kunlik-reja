...
class OpenWakeWordEngine(
    ...
    private var model: KeywordDetector? = null
    ...
    override fun start(onDetected: () -> Unit, onError: (Throwable) -> Unit) {
        val m = try {
            model ?: KeywordDetector.load(context).also { model = it }
        } catch (t: Throwable) {
            onError(t)
            return
        }
...
object WakeWordEngineFactory {
    /**
     * AUTO picks the best engine available on this device:
     *  1. Porcupine "Jarvis" when a Picovoice AccessKey is configured,
     *  2. Vosk keyword spotting "Jarvis" once the offline English model is installed (no key needed),
     *  3. the bundled openWakeWord "Hey Jarvis" model, which always works.
     */
    fun create(
        context: Context,
        choice: WakeEngineChoice,
        picovoiceKey: String?,
        sensitivity: Float,
        voskKeywordModel: VoskModel? = null
    ): WakeWordEngine {
        val hasKey = !picovoiceKey.isNullOrBlank() && picovoiceKey != "MY_PICOVOICE_ACCESS_KEY"
        val voskReady = voskKeywordModel?.isInstalled() == true
        return when (choice) {
            WakeEngineChoice.OPEN_WAKE_WORD -> OpenWakeWordEngine(context, sensitivity)
            WakeEngineChoice.VOSK -> if (voskReady) VoskKeywordEngine(context, voskKeywordModel!!, sensitivity) else OpenWakeWordEngine(context, sensitivity)
            WakeEngineChoice.PORCUPINE, WakeEngineChoice.AUTO -> when {
                hasKey -> PorcupineWakeWordEngine(context, picovoiceKey!!, sensitivity)
                voskReady -> VoskKeywordEngine(context, voskKeywordModel!!, sensitivity)
