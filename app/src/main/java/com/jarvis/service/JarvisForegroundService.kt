...
import com.jarvis.voice.WakeController
import com.jarvis.wakeword.WakeWordEngine
import com.jarvis.wakeword.WakeWordEngineFactory
...
class JarvisForegroundService : LifecycleService(), WakeController {
    ...
    override fun onCreate() {
        ...
        lifecycleScope.launch {
            container.settings.state
                .map { listOf(it.wakeEngine, it.wakeSensitivity, container.picovoiceKey(), container.voskKeywordModel.isInstalled()) }
    ...
    private suspend fun rebuildEngine() {
        ...
        val engine = WakeWordEngineFactory.create(this, s.wakeEngine, key, s.wakeSensitivity, container.voskKeywordModel)
        wakeEngine = engine
                ...
                engine.start(
                    ...
                    onError = { e ->
                        ...
                        lifecycleScope.launch(Dispatchers.Main) {
                            // Porcupine key rejected or Vosk model failed: fall back to the bundled engine once.
                            if (engine.name != "openWakeWord") {
                                wakeEngine?.release()
                                val fallback = WakeWordEngineFactory.create(this@JarvisForegroundService,
                                    com.jarvis.settings.WakeEngineChoice.OPEN_WAKE_WORD, null, container.settings.state.value.wakeSensitivity)
                                wakeEngine = fallback
    ...
    companion object {
        ...
        const val ACTION_TOGGLE_PAUSE = "com.jarvis.TOGGLE_PAUSE"
        const val ACTION_LISTEN = "com.jarvis.LISTEN"
