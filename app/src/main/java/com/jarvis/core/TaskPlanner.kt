...
data class FlexibleItem(
    ...
    /** The user named an exact time ("har kuni 7 da"): keep it even outside the working day. */
...
class TaskPlanner(
    ...
    fun plan(
        ...
        productiveHours: List<Int> = emptyList()
    ): DayPlan {
        ...
        // 3a. Habits with an exact time ("har kuni 7 da sport") are pinned before flexible work is placed,
        //     even outside the configured working day.
        val nowBound = if (date == now.toLocalDate()) roundUp(now.toLocalTime()) else LocalTime.MIN
        for (item in flexible.filter { it.exact }) {
            val start = item.preferredStart ?: continue
            if (start.isBefore(nowBound)) continue
            val end = safePlus(start, item.durationMinutes.toLong())
            if (blocks.any { overlaps(start, end, it.start, it.end) }) continue
            blocks += PlanBlock(item.title, start, end, BlockKind.HABIT)
        }

        // 3b. Overdue / missed tasks by deadline, priority, then duration.
        val flexibleTasks = (missed + overdue.filter { !it.isCompleted && it.id !in pendingToday.map { p -> p.id } })
            .sortedWith(compareBy<Task> { it.deadline.ifBlank { "9999-12-31" } }
                .thenByDescending { priorityLevel(it.priority) }
                .thenBy { it.durationMinutes })
        // Important work goes into the hours the user has proven most productive in.
        val focusStart = productiveHours.firstOrNull { it >= earliest.hour && it < dayEnd.hour }?.let { LocalTime.of(it, 0) }
            ?.let { if (it.isBefore(earliest)) earliest else it }
        val unscheduled = mutableListOf<Task>()
        for (task in flexibleTasks) {
            val urgent = task.priority == "HIGH" || (task.deadline.isNotBlank() && task.deadline <= date.plusDays(1).toString())
            val slot = findSlot(blocks, earliest, dayEnd, task.durationMinutes.toLong(), if (urgent) focusStart else null)
        ...
        // Exact-time habits whose time already passed today are not re-slotted.
        flexible.filter { !it.exact }.forEach { item ->
            val slot = findSlot(blocks, earliest, dayEnd, item.durationMinutes.toLong(), item.preferredStart) ?: return@forEach
