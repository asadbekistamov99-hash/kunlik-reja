...
| **Voice** | Speech-to-text through Google (Android `SpeechRecognizer`, on-device when offline), OpenAI **Whisper**, or **Vosk** (offline Uzbek model). It falls back between them automatically, including when a recognition service refuses background use. You choose a **male or female voice**. With a Gemini or OpenAI key, replies use natural **neural voices** (Gemini "Charon"/"Kore", OpenAI "onyx"/"nova"). Otherwise Jarvis picks the best installed device voice for your gender (Uzbek → Turkish → Russian → default), shapes pitch, and pauses naturally between sentences. |
| **Understanding** | Restores the o'/g' apostrophes that recognizers drop ("qosh" → "qo'sh"). A spell-correction pass snaps misheard words onto the command vocabulary when the first reading is weak. Of the recognizer's up to 5 guesses, Jarvis acts on the one it understands best. Longer end-of-speech silence stops long commands being cut off. |
| **Hands-free** | Once switched on (automatically on first launch) Jarvis listens 24/7: say **"Hey Jarvis"** with the app closed or the phone locked. It can also be the phone's **default assistant** (long-press power/home, headset button), and there is a **Quick Settings tile**. None of these open the app UI. |
