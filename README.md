...
| Area | What it does |
|---|---|
| **Offline wake word** | Bundled **openWakeWord "hey Jarvis"** ONNX model (no key, no network). If you add a Picovoice AccessKey it switches to **Porcupine's built-in "Jarvis"** keyword. Silent frames skip the neural stages to save battery. |
...
| **Memory** | Long-term memory for your profile, habits, preferences and important facts. It also keeps a conversation and command history and short-term context. "Men har kuni ertalab sport qilaman" is stored permanently, a habit tracker is created, and a daily reminder is set. |
...
├── jarvis/
│   ├── core/          JarvisEngine, IntentResolver, CommandParser, ActionExecutor, TaskPlanner, GeminiAgent, Ports
│   ├── wakeword/      WakeWordEngine (Porcupine / openWakeWord), JarvisKeywordModel (ONNX), AudioListener
│   ├── voice/         SpeechToText (Google, Whisper, Vosk + router), TextToSpeech, VoiceSessionManager
│   ├── memory/        MemoryDatabase (Room), LongTermMemory, UserProfile, ContextMemory, ConversationHistory, BackupManager
│   ├── integrations/  GoogleAuth, GoogleCalendar, Gmail, FileManager, CameraController, Contacts, Notifications
│   ├── automation/    ReminderEngine, HabitEngine, SmartPlanner
│   ├── service/       JarvisForegroundService, JarvisServiceController
│   ├── security/      SecureStore, DatabaseEncryption, PermissionManager, BiometricGate
│   ├── settings/      JarvisSettings (user_settings table)
│   └── AppContainer   dependency graph shared by UI, service and receivers
├── data/              Room entities, DAOs, migrations (v3 → v4)
├── receiver/          TaskReminderReceiver, BootReceiver
└── ui/                theme, components (orb, waveform, glass), navigation, 7 screens, view models
```
...
- **Instrumented (CI emulator matrix: Android 12, 13, 14, 15):** encrypted DB on disk, navigation, the foreground service, the locked screen, battery saver, no-internet mode and the reboot recovery path.

## Platform notes

- Android 14 and later forbid starting a microphone foreground service from `BOOT_COMPLETED`. On those versions Jarvis posts a one-tap "Jarvis tayyor" notification after a reboot. Earlier versions restart it directly.
